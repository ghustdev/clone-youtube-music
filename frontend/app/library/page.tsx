...
export default function Library() {
  ...
  // Estado para armazenar as músicas curtidas
  const [likedMusics, setLikedMusics] = useState<any[]>([]);

  useEffect(() => {
    if (!isAuthenticated) {
      router.replace("/auth");
    } else {
      // Busca as curtidas salvas no navegador
  ...
  return (
    <div className="flex flex-col gap-8 pb-32">
      ...
      <div className="flex flex-col gap-8">
                ...
                <Link href={`/album/${music.id}`} key={music.id}>
                  <div className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-3 rounded-lg hover:bg-zinc-800 transition-colors group cursor-pointer">
                    <div className="w-16 h-16 bg-zinc-800 rounded relative overflow-hidden flex-shrink-0">
                      <img 
                        src="https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?auto=format&fit=crop&q=80&w=150&h=150" 
                        alt="Capa" 
                    ...
                    <div className="flex flex-col overflow-hidden">
                      <strong className="text-white truncate">{music.title}</strong>
                      <span className="text-sm text-zinc-400 truncate">{music.artist}</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {/* SEÇÃO 2: PLAYLISTS SALVAS (Mock Visual) */}
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
